// app/api/upload-character-image/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const blobKey = `projects/${projectId}/characters/${index}-${Date.now()}.${extension}`
    ...
    const kvKey = `ipbible:images:${projectId}:characters`
    await kv.hset(kvKey, { [String(index)]: blobUrl })

    return NextResponse.json({ url: blobUrl }, { status: 200 })
  } catch (err: any) {
    console.error("upload-character-image error:", err)
    return NextResponse.json(
      { error: err?.message || "Character image upload failed." },
      { status: 500 }
    )
  }
}
