...
export default function CharactersPage() {
  ...
  useEffect(() => {
    ...
    const list = (bible.characters && bible.characters.list) || []
    setCharacters(list as CharacterCardData[])
  ...
  // Supports keys as numeric indexes OR character IDs
  useEffect(() => {
    if (!projectId) return
    ...
    ;(async () => {
      try {
        ...
        Object.entries(raw).forEach(([k, v]) => {
          let idx = Number(k)
          if (Number.isNaN(idx)) {
            const foundIndex = characters.findIndex((c) => c.id === k)
            if (foundIndex !== -1) {
              idx = foundIndex
            }
          }
        ...
        console.error("load character images error:", err)
      }
    })()
  }, [projectId, characters])

  const showTemplate = characters.length === 0

  async function handleGeneratePortrait(index: number, c: CharacterCardData) {
    ...
    try {
      const res = await fetch("/api/generate-character-image", {
        ...
        body: JSON.stringify({
          ...
          description:
            c.bio ||
            [c.occupation, c.role].filter(Boolean).join(" · ") ||
            "",
      ...
      if (!res.ok) {
        throw new Error(
          data?.error || "Image generation failed on the server."
        )
      }

      if (!data.url) {
        throw new Error("Image API did not return a URL.")
      }
      ...
      setErrors((prev) => ({
        ...prev,
        [index]: err?.message || "Image generation failed.",
      }))
    } finally {
  ...
  async function handleUploadImage(index: number, file: File) {
    ...
    try {
      ...
      formData.append("kind", "characters")
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1 style={{ fontSize: "2rem", marginBottom: "0.5rem" }}>Characters</h1>
        ...
        >
          One trading-style card per character. Generate or upload a portrait
          and click a card to open the full character sheet.
        </p>
      ...
      >
        {showTemplate && (
          ...
          >
            ...
            >
              ...
              >
                Short bio: who they are, what they want, and why we care.
              </p>
            </div>
          </div>
        )}
        ...
        {characters.map((c, index) => {
          ...
          return (
            <div key={c.id || index} style={{ position: "relative" }}>
              <input
                type="file"
                accept="image/*"
               ref={(el) => {
              ...
              >
                ...
                >
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault()
                      void handleGeneratePortrait(index, c)
                    }}
